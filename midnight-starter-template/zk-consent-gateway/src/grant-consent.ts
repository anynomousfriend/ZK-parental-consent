 ...
 */
async function hashIdentifier(identifier: string): Promise<bigint> {
...
async function main() {
    ...
    const { wallet, shieldedSecretKeys, dustSecretKey, unshieldedKeystore } =
        await withStatus("Restoring wallet", () => buildWallet(seed));

    // --- Wait for sync ---
    const syncedState = await withStatus("Syncing wallet", () =>
        Rx.firstValueFrom(
            wallet.state().pipe(
    ...
    // --- Find Contract ---
    const contract = await withStatus("Finding deployed contract", () =>
