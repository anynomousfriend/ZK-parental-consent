...
import { type ConsentProviders, type DeployedConsentContract } from './common-types';
...
║          ────────────────────────────                        ║
║          A privacy-preserving consent management system      ║
║                                                              ║
╚══════════════════════════════════════════════════════════════╝
`;
...
${DIVIDER}
  [1] Deploy a new consent contract
  [2] Join an existing consent contract
  [3] Monitor DUST balance
  [4] Exit
${'─'.repeat(62)}
> `;

/** Build the consent actions menu, showing current DUST balance in the header. */
const consentMenu = (dustBalance: string) => `
${DIVIDER}
  Consent Actions${dustBalance ? `                     DUST: ${dustBalance}` : ''}
${DIVIDER}
  [1] Grant consent
  [2] Verify consent
  [3] Exit
...
const joinContract = async (providers: ConsentProviders, rli: Interface): Promise<DeployedConsentContract> => {
  const contractAddress = await rli.question('Enter the contract address (hex): ');
  return await api.joinConsentContract(providers, contractAddress);
};
...
const deployOrJoin = async (
  ...
  rli: Interface,
): Promise<DeployedConsentContract | null> => {
  while (true) {
    ...
    switch (choice.trim()) {
      case '1':
        try {
          const contract = await api.withStatus('Deploying consent contract', () =>
            api.deployConsent(providers, {}),
          );
 ...
 * can grant/verify consent.
 */
const mainLoop = async (providers: ConsentProviders, walletCtx: api.WalletContext, rli: Interface): Promise<void> => {
  const consentContract = await deployOrJoin(providers, walletCtx, rli);
  ...
  while (true) {
    const dustLabel = await getDustLabel(walletCtx.wallet);
    const choice = await rli.question(consentMenu(dustLabel));
    switch (choice.trim()) {
      case '1':
        try {
          const childId = await rli.question('Enter child ID to grant consent for: ');
          const childIdHash = BigInt('0x' + Buffer.from(childId).toString('hex'));
          await api.withStatus('Granting consent', () => api.grantConsent(consentContract, childIdHash));
          console.log(`  ✓ Consent granted for child ID: ${childId}\n`);
        } catch (e) {
          const msg = e instanceof Error ? e.message : String(e);
          console.log(`  ✗ Grant consent failed: ${msg}\n`);
        }
        break;
      case '2':
        try {
          const childId = await rli.question('Enter child ID to verify: ');
          const childIdHash = BigInt('0x' + Buffer.from(childId).toString('hex'));
          const result = await api.withStatus('Verifying consent', () => api.verifyMinorAccess(consentContract, childIdHash));
          console.log(`  ${result.hasConsent ? '✓ AUTHORIZED' : '✗ NOT AUTHORIZED'}\n`);
        } catch (e) {
          const msg = e instanceof Error ? e.message : String(e);
          console.log(`  ✗ Verify consent failed: ${msg}\n`);
        }
        break;
      case '3':
        return;
 ...
 *   4. Enter the contract deploy/join and consent interaction loop
...
export const run = async (config: Config, _logger: Logger, dockerEnv?: DockerComposeEnvironment): Promise<void> => {
  ...
  try {
    ...
    if (dockerEnv !== undefined) {
      ...
      if (config instanceof UndeployedConfig) {
        config.indexer = mapContainerPort(env, config.indexer, 'consent-indexer');
        config.indexerWS = mapContainerPort(env, config.indexerWS, 'consent-indexer');
        config.node = mapContainerPort(env, config.node, 'consent-node');
        config.proofServer = mapContainerPort(env, config.proofServer, 'consent-proof-server');
      }
    }
