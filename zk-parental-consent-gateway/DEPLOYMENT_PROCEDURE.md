...
3. Verify you see:
   ...
   > cd zk-consent-gateway && npm run build
...
```
https://your-app.vercel.app/midnight/consent/keys/grant_consent.verifier
```
...
3. npm run build-production
   ...
   3a. cd zk-consent-gateway && npm run build
       → Compiles contract
       → Generates/copies keys to zk-consent-gateway/dist/managed/

   3b. cd ../frontend-vite-react && npm run build

       3b.1. npm run copy-contract-keys
             → Copies keys from zk-consent-gateway/src/managed/
             → To frontend-vite-react/public/midnight/consent/
...
└── midnight/
    └── consent/
        ├── keys/
        │   ├── grant_consent.prover
        │   ├── grant_consent.verifier
        │   ├── revoke_consent.prover
        │   ├── revoke_consent.verifier
        │   ├── verify_minor_access.prover
        │   └── verify_minor_access.verifier
        └── zkir/
            ├── grant_consent.zkir
            ├── grant_consent.bzkir
            ├── revoke_consent.zkir
            ├── revoke_consent.bzkir
            ├── verify_minor_access.zkir
            └── verify_minor_access.bzkir
```
