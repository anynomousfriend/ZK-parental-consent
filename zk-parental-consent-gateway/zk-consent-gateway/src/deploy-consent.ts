/**
 * Standalone deploy script for the ZK Consent Gateway contract.
 *
 * Usage:
 *   npm run build && npm run deploy:consent
 *
 * Prompts for a hex seed (or generates a new one), builds a wallet,
 * waits for sync + funds + dust, deploys the consent contract,
 * and writes deployment-consent.json with the contract address.
 */
...
async function main() {
  ...
  console.log("\n╔══════════════════════════════════════════════════════════╗");
  console.log("║        ZK Consent Gateway — Contract Deploy            ║");
  console.log(`║        Network: ${NETWORK_ID.padEnd(41)}║`);
  console.log("╚══════════════════════════════════════════════════════════╝\n");
  ...
  const zkConfigProvider = new NodeZkConfigProvider<ConsentCircuits>(
    zkConfigPath
  );

  const providers = {
    privateStateProvider: levelPrivateStateProvider<
      typeof ConsentPrivateStateId
    >({
  ...
  const contract = await withStatus(
    "Deploying ZK Consent Gateway contract (this may take a few minutes)",
    async () => {
  ...
  console.log(`\n╔══════════════════════════════════════════════════════════╗`);
  console.log(`║  ZK Consent Gateway contract deployed successfully!     ║`);
  console.log(`╚══════════════════════════════════════════════════════════╝`);
